...
def upload_memes_with_tags():
    ...
    for i, meme_info in enumerate(MEMES_DATA, 1):
        ...
        if Meme.objects.filter(title=title).exists():
            print(f"   ⚠️  Мем уже существует, пропускаем")
            skipped += 1
            continue
    ...
    if successful > 0:
        ...
        print(f"👁 Для просмотра мемов: http://127.0.0.1:8000/memes/")
        print(f"🔧 Админка: http://127.0.0.1:8000/admin/memes/meme/")
