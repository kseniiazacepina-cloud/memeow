...
class MemeViewSet(viewsets.ModelViewSet):
    ...
    queryset = Meme.objects.filter(is_published=True).select_related(
    ...
    def personal_meme_of_the_day(self, request):
        ...
        if not meme:
            ...
            if user_liked_tags.exists():
                ...
                if recommended_memes.exists():
                    ...
                    if count > 0:
                        random_index = random.randint(0, count - 1)
                        meme = Meme.objects.filter(is_published=True)[
                            random_index]
            else:
        ...
        if meme:
            ...
            return Response(serializer.data)
        return Response(
            {'detail': 'Персональный мем дня не найден'}, status=404)
...
class FavoriteViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Favorite.objects.filter(
            user=self.request.user).select_related('meme')
...
class StatsView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        meme_serializer = MemeSerializer(
            popular_memes, many=True, context={
                'request': request})
